     ...
     for (int i = 0; i < NO_OF_COURTS; i++)
    {
        if ((time_stamp + 1 > (courts_status.last_game_end_time[court_indices[i]])) && (courts_status.court[court_indices[i]] == OCCUPIED))
        {
            ...
            if (courts_status.game_running[court_indices[i]] == 0){
                    char msg[100];
                    snprintf(msg, sizeof(msg),"%d %d %d %d %d %d %d %d",0,courts_status.last_game_start_time[court_indices[i]],courts_status.last_game_end_time[court_indices[i]],court_indices[i]+1,courts_status.player_ids[court_indices[i]][0],courts_status.player_ids[court_indices[i]][1],-1,-1);
...
int main()
{
    ...
    printf("[+]Server Socket is created for thread %d.\n", tid);
    ...
    if (ret < 0)
    {
        printf("[-]Error in binding for thread %d. Maybe earlier socket is still running. Wait for some time or use different PORT\n", tid);
        exit(1);
    }
    printf("[+]Bind to port %d for thread %d\n", PORT, tid);
    if (ret < 0)
    {
        printf("[-]Error in binding.\n");
        exit(1);
