...
void write_csv(int player_id, int g_type, int start_time, int end_time, int court_no, int pl1, int pl2, int pl3, int pl4, char* all_messages)
{
...
void receiveResponse(int clientSocket, int player_id){
    ...
    if (recvStatus <= 0) {
        ...
        if (g_type == 0) {
            ...
            winning_player_id = (int *)malloc(2*sizeof(int));
            loosing_player_id = (int *)malloc(2*sizeof(int));
            switch(winner_id){
                case 1:
                    // printf("Player-ID %d and Player-ID %d won the game against Player-ID %d and Player-ID %d.\n", pl1, pl2, pl3, pl4);
                    winning_player_id[0] = pl1;
                    winning_player_id[1] = pl2;
                    loosing_player_id[0] = pl3;
                    loosing_player_id[1] = pl4;
                    break;
                case 2:
                    // printf("Player-ID %d and Player-ID %d won the game against Player-ID %d and Player-ID %d.\n", pl3, pl4, pl1, pl2);
                    winning_player_id[0] = pl3;
                    winning_player_id[1] = pl4;
                    loosing_player_id[0] = pl1;
                    loosing_player_id[1] = pl2;
                    break;
            }
            char congrats_msg[] = "Congratulations on your win!";
            char thank_you_msg[] = "Thank you for the game!";
        ...
        if (g_type == 0 || g_type == 1 || g_type == -1){
            write_csv(player_id,g_type,game_start_time,game_end_time,court_no,pl1,pl2,pl3,pl4,all_messages);
        } 
    }
    // Close the socket
    close(clientSocket);
    ...
    if (world_rank == 0) { // Master process
        ...
        if (records[record_no].arrival_time > received_arrival_time) {
            ...
            if (record_no==num_records-1){
                ...
                // printf("Waiting for processing of all the requests to be completed...\n");
                sleep(records[num_records-2].arrival_time - records[0].arrival_time+15);
            } //Do nothing
