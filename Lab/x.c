...
void generate_strings(int process_id, int num_processes, int max_length, int total_strings, char** all_strings) {
    ...
    for (string_length = 1; string_length <= max_length; string_length++) {
        ...
        for (i = start_index; i < end_index; i++) {
            ...
            for (j = 1; j < string_length; j++) {
                ...
                (*all_strings)[local_count][j] = char_set[(idx + i) % MAX_CHAR_SET];
            }
            local_count++;
