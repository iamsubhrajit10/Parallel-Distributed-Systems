...
#define MAX_CHAR_SET 52 // Total number of alphanumeric characters (without numbers)
...
void generate_unique_strings(int process_id, int num_processes, int max_length, int total_strings, char *local_buffer, int *counts) {
    char char_set[MAX_CHAR_SET + 1] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
...
int main(int argc, char *argv[]) {
    ...
    // Print the generated alphanumeric strings on each process
    for (int i = 0; i < num_processes; i++) {
        MPI_Barrier(MPI_COMM_WORLD);
        if (process_id == i) {
           // printf("Process %d generated %d alphanumeric strings:\n", process_id, counts[process_id]);
            for (int j = 0; j < counts[process_id]; j++) {
                printf("%s\n", &local_buffer[j * (N + 1)]);
            }
            fflush(stdout);
        }
