...
void generate_strings(int process_id, int num_processes, int max_length, int total_strings, char **all_permutations) {
    ...
    // Gather all permutations generated by different processes to process 0
    MPI_Gather(permutations[0], (max_length + 1) * strings_per_process, MPI_CHAR, *all_permutations, (max_length + 1) * strings_per_process, MPI_CHAR, 0, MPI_COMM_WORLD);
...
int main(int argc, char *argv[]) {
    ...
    // Allocate memory for storing all permutations in process 0
    ...
    // Generate strings
    generate_strings(process_id, num_processes, N, X, all_permutations);

    // Process 0 prints all accumulated strings
    if (process_id == 0) {
        for (int i = 0; i < X; i++) {
            printf("%s\n", all_permutations[i]);
        }
